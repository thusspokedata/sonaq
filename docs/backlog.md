...
### Vulnerabilidades transitivas de `npm audit` — 1 riesgo aceptado (adm-zip, sin parche upstream)
**Estado:** ⚠️ Al 2026-09-10 `npm audit` reporta **4 moderate**, todas el mismo advisory de `adm-zip` propagado por la cadena del **CLI de Sanity** (`adm-zip` → `@sanity/runtime-cli` → `@sanity/cli` → `sanity`). El resto de las transitivas se mantiene en **0**.
**Cómo se llegó:** primero (2026-07-31) `npm audit fix` + override `"sharp": "$sharp"` resolvieron las 3 CRITICAL de auth (`next-auth`/`@auth/core`/`@auth/prisma-adapter`), `tar`, el bypass de middleware de `next`, `postcss` top-level y el `sharp` interno de next. Después (2026-08-14) el resto de las transitivas (nanoid, js-yaml, adm-zip, uuid) se cerraron con `overrides` por-rango en `package.json`. Con el tiempo se sumaron overrides para advisories que fueron apareciendo: `deepmerge-ts` (2026-08), `browserslist` + `mysql2` (2026-08-31), `smol-toml` (2026-09-10), y se bumpeó `js-yaml` a `3.15.2`/`4.3.2` cuando el rango del advisory se expandió y alcanzó las versiones que fijábamos.
**adm-zip (riesgo aceptado):** el advisory GHSA-vwc7-r8mq-g2x9 (symlink → sobrescritura arbitraria al extraer un zip) afecta a **todas** las versiones publicadas `>=0.5.9`, incluida la última `0.6.0` — **no hay versión parcheada upstream**, así que ningún override lo cierra. Entra únicamente por el **CLI de Sanity** (herramienta de dev: deploy de studio, etc.), no por el bundle de runtime de la app (`@sanity/client`/`next-sanity`), y el exploit requiere extraer un zip malicioso — algo que no hacemos. `npm audit fix --force` lo "arreglaría" bajando `sanity` a 3.84.0 (major breaking) → descartado. Se revisa cuando adm-zip publique un fix o Sanity actualice su CLI.
**Overrides vigentes** (`package.json` → `overrides`): `sharp:$sharp`, `adm-zip:^0.6.0` (best-effort, sin fix aún), `nanoid@^3→3.3.18`/`@^5→5.1.16`, `js-yaml@^3→3.15.2`/`@^4→4.3.2`, `uuid@^9→11.1.1`/`@^10→11.1.1`, `deepmerge-ts:^8.0.1`, `browserslist:^4.28.8`, `mysql2:^3.24.3`, `smol-toml:^1.8.0`. Validado: build OK + 53 tests verdes.
**Regla que se mantiene:** NO correr `npm audit fix --force` (quiere *downgradear* `next`, `sanity` y `prisma` a majors viejos). Si reaparecen advisories transitivas con parche, aplicar el mismo patrón de override por-rango antes que un major.
